...
import type { Column, Lang, TableData } from "./types";
...
const HEADER_WORD_RE =
  /^\s*(순번|번호|no\.?|number|영어|english|word|단어|한글|한국어|korean|뜻|의미|meaning)\s*$/i;
...
function runPaddleScript(pythonPath: string, imagePath: string): Promise<string> {
  return new Promise((resolve, reject) => {
    let child;
    try {
      child = spawn(pythonPath, [SCRIPT_PATH, imagePath], { windowsHide: true });
    } catch (err) {
      reject(err);
      return;
    }
    ...
    const timer = setTimeout(() => {
      ...
      reject(
        new Error(
          "PaddleOCR 처리가 10분 안에 끝나지 않아 중단했습니다. 최초 실행이라면 모델 다운로드 때문일 수 있으니 잠시 후 다시 시도해주세요."
        )
      );
    }, TIMEOUT_MS);
    ...
    child.once("error", (err: NodeJS.ErrnoException) => {
      ...
      reject(
        err.code === "ENOENT"
          ? new Error(
              `Python 실행 파일(${pythonPath})을 찾을 수 없습니다. Python이 설치되어 있는지 확인하거나, 설정에서 Python 경로를 지정해주세요.`
            )
          : err
      );
    });
    child.once("close", (code) => {
      ...
      if (stdout.trim()) {
        ...
        reject(
          new Error(
            `PaddleOCR 스크립트가 응답 없이 종료되었습니다 (exit ${code}).` +
...
function parseScriptOutput(stdout: string): { cells?: unknown; error?: unknown } {
  const line = stdout
    .split(/\r?\n/)
    .reverse()
...
function classifyLang(values: string[]): Lang {
...
function looksLikeHeaderRow(rows: string[][]): boolean {
  ...
  if (rows[0].some((cell) => HEADER_WORD_RE.test(cell))) return true;
...
export async function extractTableWithPaddle({
  dataUrl,
  pythonPath,
}: {
  dataUrl: string;
  ...
  try {
    ...
    const cells: string[][] = rawCells.map((row: unknown[]) =>
