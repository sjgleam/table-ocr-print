...
이 앱은 다음과 같은 **3열 단어표** 사진을 기준으로 만들고 테스트했습니다.
...
- 왼쪽부터 **순번(번호) → 영어 → 한글 뜻** 순서의 표
- 순번 열은 인식 후 자동으로 인식되지만, 인쇄/PDF 출력에는 포함되지 않습니다(필요 없는 정보라 항상 제외).
- 영어 또는 한글에 해당하는 열이 여러 개(예: 단어+숙어처럼 같은 언어 열이 2개 이상)여도 각 열마다 답 쓰는 빈 칸이 따로 붙어서 나옵니다.
- 사진 한 장에 표가 여러 개 있거나 위 구조와 많이 다른 표는 인식 정확도가 떨어질 수 있습니다.
